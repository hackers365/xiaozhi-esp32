...
### 1.2 服务器主动唤醒流程（仅适用于MQTT协议）

**注意**：WebSocket协议没有长连接，设备在idle状态时WebSocket连接是断开的，服务器无法主动发送消息。服务器主动唤醒功能主要适用于MQTT协议场景。

WebSocket协议下的限制：
- 设备在idle状态时，WebSocket连接已断开
- 服务器无法通过WebSocket主动发送speak_request消息
- 只有用户主动唤醒（按键、语音唤醒）才能建立WebSocket连接

如果需要服务器主动唤醒功能，请使用MQTT+UDP协议（详见`mqtt-udp_zh.md`文档）。
